...
class RoutingAgent:
    ...
    async def send_message(self, agent_name: str, task: str, tool_context: ToolContext):
        """Send a message to a remote agent. Creates a new task on first call,
        then continues the SAME task on subsequent calls using the server's task_id.
        """
        ...
        # --- IMPORTANT: only reuse task/context if we ALREADY have them ---
        task_id = state.get('task_id')          # don't invent a new one
        context_id = state.get('context_id')    # don't invent a new one

        # message id is always new
        message_id = str(uuid.uuid4())

        # Build payload: only include taskId/contextId if continuing
        payload = {
            'message': {
                'role': 'user',
        ...
        # Simple visibility for routing
        try:
            agent_url = getattr(client, "agent_url", None) or "UNKNOWN_URL"
        except Exception:
            agent_url = "UNKNOWN_URL"
        print(f"[route] -> send to '{agent_name}' @ {agent_url}  (continuation={bool(task_id)})")
        ...
        # Log raw response (trimmed)
        print("send_response", send_response.model_dump_json(exclude_none=True, indent=2))

        # Must be success + Task payload
        if not isinstance(send_response.root, SendMessageSuccessResponse):
            print('received non-success response. Aborting.')
            return None
        if not isinstance(send_response.root.result, Task):
            print('received non-task response. Aborting.')
            return None

        # --- Capture server-assigned ids for future turns ---
        task_obj: Task = send_response.root.result

        # Some schemas use 'id', some use 'taskId'—cover both.
        returned_task_id = getattr(task_obj, "id", None) or getattr(task_obj, "task_id", None)
        if returned_task_id:
            state['task_id'] = returned_task_id

        returned_context_id = getattr(task_obj, "context_id", None)
        if returned_context_id:
            state['context_id'] = returned_context_id

        print(f"[route] <- got task_id={state.get('task_id')}  context_id={state.get('context_id')}")
